...
class Outer<T> {
    ...
    fun test() {
        ...
        <!UNRESOLVED_REFERENCE_WRONG_RECEIVER!>NestedTAToIntInner<!>() // Error (different dispath receivers `Outer<T>` and `Outer<Int>`)
        <!UNRESOLVED_REFERENCE_WRONG_RECEIVER!>NestedTAWithTypeParameterToInner<!><String>() // Error (different dispath receivers `Outer<T>` and `Outer<String>`)
        NestedTAtoInnerWithTypeParameters() // Error (minor, not inner typealias)
        <!UNRESOLVED_REFERENCE_WRONG_RECEIVER!>NestedTAtoIntInnerWithTypeParameters<!>() // Error (different dispath receivers `Outer<T>` and `Outer<Int>`)
    }
}
